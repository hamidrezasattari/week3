...
Super Mastermind
8 color	5 hole,
In MasterMind Deluxe the player must guess a code by placing pegs.
The goal is to arrange different colored pegs in the correct order in a row.
 The resulting pattern must match the hidden code of the coder at the end.

 It consists of 5 pins, which in turn can be selected from 8 colors.
...
template MastermindVariation() {
    ...
    // Create a constraint that the solution and guess digits are all less than 8 (color code 0-7).
    for (j=0; j<5; j++) {
    ...
    // validate actual vs generated  hit
    component equalHit = IsEqual();
    equalHit.in[0] <== pubNumHit;
    equalHit.in[1] <== hit;
    equalHit.out === 1;

    // validate actual vs generated blow
